...
현재 checkpoint는 **M1.7 Local Product Acceptance & Freeze 완료**입니다. Versioning과 binary publication은 M1.7 완료와
별도로 승인해야 하는 gate입니다.
...
| Public source publication | 검토된 M1.7 source snapshot 공개 | 검토 중 |
...
### 1. Public source publication

검토된 M1.7 commit을 기준으로 complete source, test, build metadata와 공개용 문서를 제공합니다. Private fixture, credential,
내부 작업 문서와 local infrastructure 설정은 포함하지 않습니다.

### 2. Versioning과 binary publication gate

버전 표기, clean staging, 실제 배포 DLL/plugin/codec 목록, Qt와 third-party notice, source 제공 방법을 별도로 검증한 뒤에만
binary publication을 진행합니다.

### 3. Product interaction 보완
