...
public:
    ...
    [[nodiscard]] virtual ExportPreparationResult prepare(const ExportRequest& request,
    ...
    [[nodiscard]] virtual ExportItemResult executeItem(const PreparedExportItem& item,
                                                       const types::CancellationToken& cancellationToken) const;
...
public:
    ...
    explicit FileExportPipeline(const platform::IPathIdentityService& pathIdentityService) noexcept;
...
private:
    const platform::IPathIdentityService* m_pathIdentityService;
